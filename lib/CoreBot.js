    ...
    function Conversation(task, message) {
        ...
        this.evaluateBotOperationEntity = async function(entity) {
            ...
            switch ((entity.type || '').toLowerCase()) {
                ...
                case 'attribute': {
                    const  value = await this.getVar(entity.attribute && entity.attribute.name, true);
        ...
        this.evaluateBotOperation = async function(operation) {
            ...
            const firstValue = await this.evaluateBotOperationEntity(operation.firstEntity);
            const secondValue = await this.evaluateBotOperationEntity(operation.secondEntity);

            if((firstValue !== 0 && !firstValue) || (secondValue !== 0 && !secondValue)) {
                return null;
            }
                    ...
                    if (this.messages.length) {
                        ...
                        if (typeof(this.messages[0].timestamp) == 'undefined' ||
                            ...
                            if(message.setVar) {
                                try {
                                    if (message.setVar.isUseOperation) {
                                        const operationResult = await this.evaluateBotOperation(message.setVar.operation);

                                        this.setVar(message.setVar.key, operationResult, message.setVar.isPersist);
                                    } else {
                                        if(message.setVar.valueEntity) {
                                            const evaluatedValue = await this.evaluateBotOperationEntity(message.setVar.valueEntity);

                                            this.setVar(message.setVar.key, evaluatedValue, message.setVar.isPersist);
                                        } else {
                                            this.setVar(message.setVar.key, message.setVar.value, message.setVar.isPersist);
                                        }
                                    }
                                } catch (error) {
                            ...
                            if(message.jsonApi) {                    
                                ...
                                message.jsonApi(attributeObjects).then(function(data) {
                                    if (!data) {
                                        return;
                                    }
                                    ...
                                    outbound.text = err.pluginMessage || pluginMessages.errorOccured || 'An error occured with a request.';
                            ...
                            const operateFilter = function (filterOperation, filterValue, attributeValue) {
                                switch (filterOperation) {
                                    case 'is':
                                        if (String(filterValue) === String(attributeValue)) {
                                            return true;
                                        }
                                    ...
                                    case 'is_not':
                                        ...
                                        return false;
                                    case 'greater_than':
                                        if (Number(attributeValue) > Number(filterValue)) {
                                            return true;
                                        }
                            ...
                            if (message.gotoDialogue) {
                                await (async (gotoDialogue) => {
                                    const { filters, allowRandom, logicalOperator, dialogueGroups } = gotoDialogue;
                                    let conditionIsTrue = (filters.length < 1) ? true : false;

                                    for (let filter of filters) {
                                        if (filter.filterBy.toLowerCase() !== 'attribute') {
                                            continue;
                                        }
                                        ...
                                        if (logicalOperator.toLowerCase() === 'or') {
                                            ...
                                            if (operateFilter(filter.filterOperation, filter.filterValue, attributeValue)) {
                                    ...
                                    message.text = dialogueGroups[randomIndex];
                                    this.stop();
                                    this.task.botkit.trigger('custom_trigger', [this.task.bot, message]);

                                    return;
                                })(message.gotoDialogue);
                            }
                            ...
                            if (message.linkToSubscription) {
                                ((linkToSubscription) => {
                                    const { filters, allowRandom, loopbackUrl, helperApiUrl, logicalOperator, subscriptionGroups } = linkToSubscription;
                                    ...
                                    for (let filter of filters) {
                                        ...
                                        const attributeValue = this.getVar(filter.filterItem.name);
                                    ...
                                    if (!allowRandom) {
                                        message.callScheduler(subscriptionGroups[0]);

                                        return;
                                    }

                                    const randomIndex = Math.floor(Math.random() * subscriptionGroups.length);

                                    message.callScheduler(subscriptionGroups[randomIndex]);

                                    return;
                                })(message.linkToSubscription);
                            }
