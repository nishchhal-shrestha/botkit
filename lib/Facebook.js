...
const getRateLimitedFunction = (func, rateInMilliSeconds) => {
    const queue = [];
...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.send = getRateLimitedFunction(function(message, cb) {
            ...
            request({
                ...
                function(err, res, body) {
                    ...
                    if (!body) {
                        botkit.debug('API ERROR', 'None or invalid body received');
                        return cb && cb({ message: 'None or invalid body received', body: body });
                    }
