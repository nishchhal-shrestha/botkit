...
const setMessageTagAndType = (tag = '', message) => {
    if (!tag || tag.toLowerCase() === 'standard') {
        message.messaging_type = messagingTypes.update;

        return;
    }

    message.messaging_type = messagingTypes.messageTag;
    message.tag = tag;
            ...
            bot.createConversation(message, async function(err, convo) {
                ...
                const updateUserProfile = async function() {
                    if (bot && bot.getMessageUser) {
                        ...
                        try {
                            const response = await bot.getMessageUser(messageCopy);

                            userInfo = {
                                user_id: messageCopy.user,
                                fb_id: response.id,
                                fb_username: response.username,
                                fb_fullname: response.full_name,
                        ...
                        try {
                            await controller.storage.users.save(userInfo);
                        } catch (error) {
                        ...
                        // no point in returning just user_id so not returning at all
                    }
                };
                ...
                if (command.isStarterTrigger) {
                    if (userData) {
                        // user is already in database no need to wait before proceeding
                        updateUserProfile();
                    } else {
                        // user needs to be stored so need to wait as below codes may try to access
                        // the user attributes
                        userData = await updateUserProfile();
                    }
                }
                ...
                if (userData) {
                    for (let attribute of commonAttributes) {
                        const attributeMatch = userData[attribute];
                ...
                if (command.variables && command.variables.length) {
                    for (var v = 0; v < command.variables.length; v++) {
                        if (userData) {
                            const attributeArray = userData[command.variables[v].name];
                            ...
                            if (Array.isArray(attributeArray) && attributeArray.length) {
                                ...
                                // attributeArray could be non-array value
                                convo.setVar(command.variables[v].name, attributeArray);
                            }
                        }
                    ...
                    for (var m = 0; m < command.script[t].script.length; m++) {
                        if (command.script[t].script[m].conditional) {
                            ...
                            if (messageCopy.isSubscription) {
                                setMessageTagAndType(command.messageTag, message)
                            }
                            ...
                            const dialogueId = command.script[t].script[m].dialogueId;
                            const dialogueName = command.script[t].script[m].dialogueName;
                            if (dialogueId || dialogueName) {
                                message.dialogueId = dialogueId;
                                message.dialogueName = dialogueName;
                            }
                            ...
                            if (bot.type == 'ciscospark') {
                                if (
                                    ...
                                    if (command.script[t].script[m].platforms.ciscospark.files) {
                                        ...
                                        for (
                                            var f = 0;
                                            f < command.script[t].script[m].platforms.ciscospark.files.length;
                                            f++
                                        ) {
                                            message.files.push(
                                                command.script[t].script[m].platforms.ciscospark.files[f].url
                                            );
                                        }
                                    }
                                }
                            ...
                            if (bot.type == 'web' || bot.type == 'socket') {
                                if (
                                    ...
                                    if (command.script[t].script[m].platforms.web.files) {
                                        ...
                                        for (
                                            var f = 0;
                                            f < command.script[t].script[m].platforms.web.files.length;
                                            f++
                                        ) {
                                            // determine if this is an image or any other type of file.
                                            command.script[t].script[m].platforms.web.files[f].image =
                                                command.script[t].script[m].platforms.web.files[f].url.match(
                                                    /\.(jpeg|jpg|gif|png)$/i
                                                ) != null;
                            ...
                            if (bot.type == 'teams') {
                                if (
                                    ...
                                    if (command.script[t].script[m].platforms.teams.attachments) {
                                        ...
                                        for (
                                            var a = 0;
                                            a < command.script[t].script[m].platforms.teams.attachments.length;
                                            a++
                                        ) {
                                        ...
                                        if (attachment.template_type == 'button') {
                                            ...
                                            for (let buttonIndex in attachment.buttons) {
                                                if (attachment.buttons[buttonIndex].type == 'web_url') {
                                                    buttonsText +=
                                                        '\n\n[' +
                                                        attachment.buttons[buttonIndex].title +
                                                        '](' +
                                                        attachment.buttons[buttonIndex].url +
                                                        ')';
                                                }
                                            }
                            ...
                            if (command.script[t].script[m].setVar) {
                                message.setVar = command.script[t].script[m].setVar;
                            }

                            if (command.script[t].script[m].jsonApi) {
                                message.jsonApiData = command.script[t].script[m].jsonApi;
                                const { apiUrl, requestType, propertyObjects, bodyFormat } = message.jsonApiData;
                                ...
                                message.jsonApi = function(newAttributeObjects) {
                                    ...
                                    return new Promise(function(resolve, reject) {
                                        ...
                                        if (bodyFormat === 'application/json') {
                                            ...
                                            headerObject['content-type'] = bodyFormat;
                                        } else {
                            ...
                            if (command.script[t].script[m].linkToSubscription) {
                                ...
                                const loopbackUrl = url.resolve(
                                    controller.config.studio_command_uri,
                                    'api/BotSubscriptions/getSubscriptionDialogues'
                                );
                                    ...
                                    return new Promise(function(resolve, reject) {
                                        ...
                                        request(options, function(err, res, body) {
                                            ...
                                            if (!json || json == null) {
                                                ...
                                                console.error('response from subscription adding api', json);
                                                resolve(json);
                                            }
                                        });
                                    });
                            ...
                            if (command.script[t].script[m].backToBot) {
                                message.backToBot = command.script[t].script[m].backToBot;
                            }
                            ...
                            if (command.script[t].script[m].collect) {
                                ...
                                if (command.script[t].script[m].collect.key) {
                                    ...
                                    capture_options.validationRegex =
                                        command.script[t].script[m].collect.validationRegex;
                                    capture_options.validationMessage =
                                        command.script[t].script[m].collect.validationMessage;
                                    capture_options.checkIfAttributeExists =
                                        command.script[t].script[m].collect.checkIfAttributeExists;
                                }

                                const cancelInputString = '##cancel**input**loop##';
                                const cancelButtonName = command.script[t].script[m].collect.cancelButtonName;
                                const allowCancelingConversation =
                                    command.script[t].script[m].collect.allowCancelingConversation;
                                    ...
                                    handlers.push({
                                        ...
                                        callback: function(r, c) {
                                            runHooks(
                                                ...
                                                convo,
                                                function(convo) {
                                                    let userResponse = c.responses[capture_options.key].text;
                                                    
                                                    if(!userResponse) {
                                                        const attachments = c.responses[capture_options.key].attachments;

                                                        try {
                                                            if(attachments && attachments.length) {
                                                                const attachment = attachments[0];

                                                                userResponse = attachment.payload.url;
                                                            }
                                                        } catch (error) {
                                                            console.error(error);
                                                        }
                                                    ...
                                                    if (!capture_options.validationRegex) {
                                                        // TODO: Check if using just setVar with isPersist flag works
                                                    ...
                                                    if (
                                                        capture_options.key.toLowerCase() !== 'none' &&
                                                        !validationRegex.test(userResponse)
                                                    ) {
                                                        const dialogueId =
                                                            convo.lastMessage && convo.lastMessage.dialogueId;
                                                        const dialogueName =
                                                            convo.lastMessage && convo.lastMessage.dialogueName;

                                                        if (dialogueId || dialogueName) {
                                                            delete convo.lastMessage.dialogueId;
                                                            delete convo.lastMessage.dialogueName;
                                                        }

                                                        convo.messages.unshift(convo.lastMessage);
                                                        convo.addMessage(
                                                            { text: capture_options.validationMessage },
                                                            topic,
                                                            1
                                                        );
                                                    } else {
                                ...
                                const {
                                    ...
                                    quickReplies,
                                    saveTemporarily
                                } = command.script[t].script[m].quickReply;
                                        ...
                                        callback: function(r, c) {
                                            runHooks(
                                                ...
                                                function(convo) {
                                                    ...
                                                    if (saveToAttribute) {
                                                        c.setVar(saveToAttribute, userResponseText, !saveTemporarily);
                                                    }
                                                    ...
                                                    if (
                                                        dialogueTriggers &&
                                                        dialogueTriggers.indexOf(userResponsePayload) > -1
                                                    ) {
                                                        const message = {};
                                                        message.user = message.channel = previousMessage.channel;
                                ...
                                const capture_options = {};
                                const shareLocationMessage =
                                    command.script[t].script[m].shareLocation.instructionMessage;
                                            ...
                                            runHooks(
                                                ...
                                                async function(convo) {
                                                    ...
                                                    const responseAttachment =
                                                        userResponse &&
                                                        userResponse.attachments &&
                                                        userResponse.attachments[0];

                                                    if (!responseAttachment) {
                                                        console.error(
                                                            'response attachment not received from send location button'
                                                        );
                                                        ...
                                                        if (responseText && responseUser) {
                                                            ...
                                                            return controller.trigger('message_received', [
                                                                bot,
                                                                message
                                                            ]);
                                                        }
                                                    ...
                                                    const latitude =
                                                        responseAttachment.payload &&
                                                        responseAttachment.payload.coordinates &&
                                                        responseAttachment.payload.coordinates.lat;
                                                    const longitude =
                                                        responseAttachment.payload &&
                                                        responseAttachment.payload.coordinates &&
                                                        responseAttachment.payload.coordinates.long;
                                                    ...
                                                    if (mapUrl) {
                                                        ...
                                                        console.error(
                                                            'map url could not be extracted from send location response'
                                                        );
                                                    }
                                                    ...
                                                    if (latitude && longitude) {
                                                        ...
                                                        console.error(
                                                            'latitude and longitude could not be extracted from send location response'
                                                        );
                                                    }
