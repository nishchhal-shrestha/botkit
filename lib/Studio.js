                        ...
                        if (command.script[t].script[m].conditional) {
                            ...
                            if (command.script[t].script[m].jsonApi) {
                                ...
                                message.jsonApi = function(newAttributeObjects) {
                                    ...
                                    if (Array.isArray(newAttributeObjects)) {
                                        for (attribute of newAttributeObjects) {
                                            attributes[attribute.key] = attribute.value;
                                        }
                                    }

                                    if (Array.isArray(propertyObjects)) {
                                        for(property of propertyObjects) {
                                    ...
                                    return new Promise(function (resolve, reject) {
                                        let options = {};
                                        options.uri = apiUrl;
                                        options.form = { ...attributes, ...properties };
                            ...
                            if (command.script[t].script[m].linkToSubscription) {
                                ...
                                message.callScheduler = function (subscriptions) {
                                    return new Promise(function (resolve, reject) {
                                        ...
                                        options.form = { subscriptions, apiUrl: loopbackUrl, user, channel };
                                        options.method = 'POST';
                            ...
                            if (command.script[t].script[m].meta) {
                             ...
                             if (command.script[t].script[m].collect) {
                                ...
                                const { saveToAttribute, dialogueTriggers } = command.script[t].script[m].quickReply;
