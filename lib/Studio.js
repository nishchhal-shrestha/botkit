                    ...
                    for (var m = 0; m < command.script[t].script.length; m++) {
                        ...
                        if (command.script[t].script[m].conditional) {
                            ...
                            if(command.script[t].script[m].jsonApi) {
                                const { apiUrl, requestType, attributeObjects } = command.script[t].script[m].jsonApi;

                                message.jsonApiData = {apiUrl, requestType, attributeObjects};
                                    ...
                                    handlers.push({
                                        ...
                                        callback: function(r, c) {
                                            ...
                                            runHooks(
                                                ...
                                                convo,
                                                function(convo) {
                                                    if(capture_options.key == "the_phone" && c.responses[capture_options.key].text == "1") {
                                                        convo.messages.unshift(convo.lastMessage);
                                                    } else {
                                                        c.setVar(capture_options.key, c.responses[capture_options.key].text);
                                                    }
                                                    c.next();
                                                }
                                            );
