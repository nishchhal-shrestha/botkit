                    ...
                    for (var m = 0; m < command.script[t].script.length; m++) {
                        ...
                        if (command.script[t].script[m].conditional) {
                            ...
                            if (command.script[t].script[m].linkToSubscription) {
                                ...
                                const loopbackUrl =
                                    url.resolve(controller.config.studio_command_uri, 'api/BotSubscriptions/getSubscriptionDialogues');
                            ...
                            if (command.script[t].script[m].meta) {
                             ...
                             if (command.script[t].script[m].collect) {
                                            ...
                                            runHooks(
                                                ...
                                                function(convo) {
                                                    ...
                                                    if (capture_options.key.toLowerCase() !== 'none' && !validationRegex.test(userResponse)) {
                                                        const dialogueId = convo.lastMessage && convo.lastMessage.dialogueId;
                                                        const dialogueName = convo.lastMessage && convo.lastMessage.dialogueName;
                                ...
                                const { saveToAttribute, dialogueTriggers, quickReplies, saveTemporarily } = command.script[t].script[m].quickReply;
